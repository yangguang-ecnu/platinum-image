...
vector<float> rendererMPR::get_values(int vx, int vy,int sx,int sy)
    {
    //virtual function, MSVC gets hickups without namespace spec however :(
    return renderer_base::get_values(view_to_unit(vx,vy,sx,sy));
    }
...
Vector3D rendererMPR::view_to_unit(int vx, int vy,int sx,int sy)
    ...
    unit=wheretorender->view_to_unit(min (sx,sy))*view;
