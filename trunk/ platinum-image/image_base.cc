...
metaloader::metaloader(std::vector<std::string> * f): imageloader(f)
{
    metaIO = itk::MetaImageIO::New();
}


image_base *metaloader::read()
{    
    image_base *result = NULL;
	...
	if(metaIO->CanReadFile(file_path.c_str())){   //Assumption: File contains image data
        metaIO->SetFileName(file_path);
        metaIO->ReadImageInformation(); 
        itk::ImageIOBase::IOPixelType pixelType=metaIO->GetPixelType();
        itk::ImageIOBase::IOComponentType componentType = metaIO->GetComponentType();
        ...
        switch(pixelType){
            case itk::ImageIOBase::SCALAR:	//Enumeration values: UCHAR, CHAR, USHORT, SHORT, UINT, INT, ULONG, LONG, FLOAT, DOUBLE
                switch(componentType){
                    case itk::ImageIOBase::UCHAR:
                        result =  new image_integer<unsigned char>();
                        ((image_integer<unsigned char>*)result)->load_dataset_from_meta_file(file_path);
                        break;
                    case itk::ImageIOBase::USHORT:
                        result = new image_integer<unsigned short>();
                        ((image_integer<unsigned short>*)result)->load_dataset_from_meta_file(file_path);
                        break;
                    case itk::ImageIOBase::SHORT:
                        result = new image_integer<short>();
						((image_integer<short>*)result)->load_dataset_from_meta_file(file_path);
                        break;
					case itk::ImageIOBase::ULONG:
                        result = new image_integer<unsigned long>();
                        ((image_integer<unsigned long>*)result)->load_dataset_from_meta_file(file_path);
                        break;
					case itk::ImageIOBase::LONG:
                        result = new image_integer<long>();
                        ((image_integer<long>*)result)->load_dataset_from_meta_file(file_path);
                        break;
                    case itk::ImageIOBase::FLOAT:
                        result = new image_integer<float>();
                        ((image_integer<float>*)result)->load_dataset_from_meta_file(file_path);
                        break;
                    case itk::ImageIOBase::DOUBLE:
                        result = new image_integer<double>();
                        ((image_integer<double>*)result)->load_dataset_from_meta_file(file_path);
                        break;
                    default:
                        pt_error::error("Load scalar meta image: unsupported component type: " + metaIO->GetComponentTypeAsString (componentType), pt_error::warning);
                    }
                break;
...
/*					case itk::ImageIOBase::UCHAR:
					...
					case itk::ImageIOBase::FLOAT:
						result = new image_complex<float>();
						((image_complex<float>*)result)->load_dataset_from_meta_file(file_path);
						break;
					#ifdef _DEBUG
					default:
						cout << "Load complex meta image: unsupported component type: " << metaIO->GetComponentTypeAsString (componentType) << endl;
					#endif
					}
                break;
			...
			case itk::ImageIOBase::VECTOR:
                switch (componentType){
					...
					#ifdef _DEBUG
					default:
						cout << "Load VECTOR meta image: unsupported component type: " << metaIO->GetComponentTypeAsString (componentType) << endl;
					#endif
					}
                break;
			...
			default:
                pt_error::error("image_base::load(...): unsupported pixel type: " + metaIO->GetPixelTypeAsString(pixelType), pt_error::warning);
            }
...
void image_base::load( std::vector<std::string> chosen_files)	//loads all files and adds them to datamanagement...
{
	...
	try_loader<metaloader>(&chosen_files);			//try Meta image
