...
//	virtual image_base* contract_borders2D_by_dir(int dir, unsigned int dr=1);
	...
	vector<image_integer<short, IMAGEDIM>* > distance_path_to_border_3D(image_binary<IMAGEDIM>* rim); //rim is the outer border of the object which are the goal points (distance = 0)
	...
	image_integer<short,IMAGEDIM>* dijkstra_image_version(image_integer<short, IMAGEDIM>* dist, vector<Vector3D> Q);
